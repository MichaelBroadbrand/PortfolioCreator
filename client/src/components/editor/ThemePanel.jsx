...
import { Sun, Moon, Check, RotateCcw, X } from 'lucide-react';
...
function ColorPickerPopover({ currentColor, onSelect, onClose, anchorRef }) {
  ...
  return createPortal(
    <>
      ...
      <div
        className="fixed z-[100] p-3 bg-surface-100 border border-white/[0.08] rounded-lg shadow-lg w-52"
        style={{ top: pos.top, left: pos.left }}
      >
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-medium text-surface-500">Pick a colour</span>
          <button type="button" onClick={onClose} className="p-0.5 hover:bg-white/[0.08] rounded text-surface-400 hover:text-surface-600">
...
export default function ThemePanel() {
  ...
  const swatchRowRef = useRef(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div ref={swatchRowRef} className="flex justify-between">
        ...
        {activeColorKey && (
          <ColorPickerPopover
            ...
            currentColor={colorScheme[activeColorKey]}
            onSelect={(c) => updateColor(activeColorKey, c)}
        ...
        <div className="space-y-1.5">
          {FONT_PAIRINGS.map((pair) => (
            <button
              key={pair.id}
              type="button"
              onClick={() => updateTheme({ fontPairing: `${pair.heading} + ${pair.body}` })}
              className={`w-full text-left px-3 py-2 rounded-lg border transition-colors ${
                theme.fontPairing === `${pair.heading} + ${pair.body}`
