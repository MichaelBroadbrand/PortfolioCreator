...
function DefaultSectionPreview({ section, theme }) {
  ...
  switch (section.type) {
      ...
      return (
        <div className="py-12 px-8">
          ...
          <div className="flex gap-6 items-start">
            ...
            <p className="text-gray-600 leading-relaxed">{content.bio || 'Tell visitors about yourself...'}</p>
          </div>
        </div>
      );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {(content.projects || []).slice(0, 4).map((p, i) => (
              <div key={i} className="border border-gray-200 rounded-lg overflow-hidden">
                ...
                <div className="p-4">
                  <h3 className="font-semibold text-gray-900">{p.title || `Project ${i + 1}`}</h3>
                  <p className="text-sm text-gray-500 mt-1">{p.description || 'Project description'}</p>
                  <div className="flex flex-wrap gap-1 mt-2">
                    {(p.tags || []).map((tag, j) => (
                      <span key={j} className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{tag}</span>
                    ))}
                  </div>
                </div>
              </div>
        ...
        <div className="py-12 px-8">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {(!content.skills || content.skills.length === 0) && (
              <p className="text-gray-400">Add your skills here</p>
            )}
          </div>
        </div>
      );
        ...
        <div className="py-12 px-8">
          ...
          {(content.entries || []).map((e, i) => (
            <div key={i} className="mb-6 pb-6 border-b border-gray-100 last:border-0">
              <h3 className="font-semibold text-gray-900">{e.company || 'Company'}</h3>
              <p className="text-sm" style={{ color: theme?.colorScheme?.primary }}>{e.role || 'Role'}</p>
              <p className="text-xs text-gray-400 mt-1">{e.startDate} — {e.current ? 'Present' : e.endDate}</p>
              <p className="text-sm text-gray-600 mt-2">{e.description || 'Description'}</p>
            </div>
          ))}
          {(!content.entries || content.entries.length === 0) && (
            <p className="text-gray-400">Add your experience here</p>
          )}
        </div>
      );
        ...
        <div className="py-12 px-8">
          ...
          {(content.entries || []).map((e, i) => (
            <div key={i} className="mb-4">
              <h3 className="font-semibold text-gray-900">{e.institution || 'Institution'}</h3>
              <p className="text-sm text-gray-600">{e.degree || 'Degree'}</p>
              <p className="text-xs text-gray-400">{e.startYear} — {e.current ? 'Present' : e.endYear}</p>
            </div>
          ))}
          {(!content.entries || content.entries.length === 0) && (
            <p className="text-gray-400">Add your education here</p>
          )}
        </div>
      );
    ...
    case 'contact':
      return (
        <div className="py-12 px-8 max-w-lg mx-auto text-center">
          ...
          <p className="text-gray-500 mb-4">{content.email || 'your@email.com'}</p>
          {(content.github || content.linkedin || content.twitter || content.website) && (
            <div className="flex justify-center gap-4 mb-4">
              {content.github && (
                <a href={content.github} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-gray-800 transition-colors">
              ...
              {content.linkedin && (
                <a href={content.linkedin} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-gray-800 transition-colors">
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                </a>
              )}
              {content.twitter && (
                <a href={content.twitter} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-gray-800 transition-colors">
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                </a>
              )}
              {content.website && (
                <a href={content.website} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-gray-800 transition-colors">
    ...
    case 'testimonials':
      return (
        <div className="py-12 px-8">
          <h2 className="text-2xl font-bold mb-6 text-gray-900">Testimonials</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {(content.testimonials || []).map((t, i) => (
              <div key={i} className="bg-gray-50 p-6 rounded-xl">
                <p className="text-sm italic text-gray-600 mb-3">"{t.quote || 'Testimonial quote'}"</p>
                <div className="flex items-center gap-2">
                  {t.avatar && (
                    <img src={t.avatar} alt={t.name} className="w-8 h-8 rounded-full object-cover" />
                  )}
                  <div>
                    <p className="text-sm font-semibold text-gray-900">{t.name || 'Name'}</p>
            ...
            {(!content.testimonials || content.testimonials.length === 0) && (
              <p className="text-gray-400">Add testimonials here</p>
            )}
          </div>
        </div>
      );
    ...
    case 'custom':
      return (
        <div className="py-12 px-8">
          <h2 className="text-2xl font-bold mb-4 text-gray-900">{content.title || 'Custom Section'}</h2>
          <div className="text-gray-600" dangerouslySetInnerHTML={{ __html: content.body || '<p>Add your content here</p>' }} />
        </div>
      );

    default:
      return (
        <div className="py-8 px-8 text-center text-gray-400">
